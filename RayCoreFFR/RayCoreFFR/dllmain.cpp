...
_declspec(dllexport) RayError RayRunImageAnalysis(unsigned char* data, int width, int height, int channels, int step) {
    return octSystem.RunImageAnalysis(data, width, height, channels, step);
}
