...
static void crc32_step(struct crc32_context *context, char byte)
{
    uint32_t j;
    //context->crc ^= byte;
    for(j = 0; j < 8; j++) {
        uint32_t b=((uint32_t)byte^(context->crc))&0x00000001;
        context->crc >>=1;
        if(b) (context->crc)=(context->crc)^0xEDB88320;
        byte>>=1;
...
{
    // Transmit the byte
    self->cb->min_tx_byte(self->port, byte);
    if(crc) {
    ...
    if(byte == HEADER_BYTE) {
        if(--self->tx_header_byte_countdown == 0) {
            self->cb->min_tx_byte(self->port, STUFF_BYTE);        // Stuff byte
...
static void on_wire_bytes(struct min_context *self, uint8_t id_control, uint8_t seq, uint8_t const *payload_base, uint16_t payload_offset, uint16_t payload_mask, uint8_t payload_len)
{
    ...
    self->cb->min_tx_start(self->port);

    // Header is 3 bytes; because unstuffed will reset receiver immediately
    self->cb->min_tx_byte(self->port, HEADER_BYTE);
    self->cb->min_tx_byte(self->port, HEADER_BYTE);
    self->cb->min_tx_byte(self->port, HEADER_BYTE);
    ...
    self->cb->min_tx_byte(self->port, EOF_BYTE);

    self->cb->min_tx_finished(self->port);
}
...
static void send_ack(struct min_context *self)
{
    ...
    if(ON_WIRE_SIZE(0) <= self->cb->min_tx_space(self->port)) {
    ...
    switch(id_control) {
        ...
        default:
            if (id_control & 0x80U) {
                ...
                if (seq == self->transport_fifo.rn || true) {
                    ...
                    self->cb->min_application_handler(id_control & (uint8_t)0x3fU, payload, payload_len, self->port);
                } else {
...
void min_poll(struct min_context *self, uint8_t const *buf, uint32_t buf_len)
{
    ...
    if((window_size < TRANSPORT_MAX_WINDOW_SIZE) && (self->transport_fifo.n_frames > window_size)) {
        ...
        if(ON_WIRE_SIZE(frame->payload_len) <= self->cb->min_tx_space(self->port)) {
        ...
        if((window_size > 0) && remote_connected) {
            ...
            if(now - oldest_frame->last_sent_time_ms >= TRANSPORT_FRAME_RETRANSMIT_TIMEOUT_MS) {
                ...
                if(ON_WIRE_SIZE(oldest_frame->payload_len) <= self->cb->min_tx_space(self->port)) {
...
{
    if((ON_WIRE_SIZE(payload_len) <= self->cb->min_tx_space(self->port))) {
