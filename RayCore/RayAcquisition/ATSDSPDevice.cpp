...
char* CATSDSPDevice::acquire(int& nCurFrame, int& nTotalFrame) {
...
BOOL CATSDSPDevice::configureFPGA(HANDLE boardHandle) {
	...
	const int nFFTLength = getFFTLength(nAScan);
...
int CATSDSPDevice::getFFTLength(unsigned int nAScan) {
	int nFFTOrder = 1;
	int nFFTLength = 1 << nFFTOrder;
	while (nFFTLength < nAScan) { // AScan 보다 큰 2^n 중에서 제일 작은 수
		nFFTOrder++;
		nFFTLength = 1 << nFFTOrder;
	}
	return nFFTLength;
}