...
//		ippsZero_32fc(ctx.fcBuffer_FFT, nFFTLength);
//		ctx.fcBuffer_IFFT = ippsMalloc_32fc(nFFTLength);
//		// fcBuffer_IFFT는 이후에 항상 Zero Pad 처리됨
//
//		// Work buffers
//		ctx.fftWorkBufFirst = ippsMalloc_8u(fftFirstWorkBufSize);
//		ctx.fftWorkBufIFFT = ippsMalloc_8u(fftIFFTWorkBufSize);
...
//		ippsMul_32f_I(calibration->window, ctx.fBuffer_Window, nFFTLength);
//
//		// (2) First FFT -> Permuted format
//		ippsFFTFwd_RToPerm_32f_I(ctx.fBuffer_Window, fftSpecFirst, ctx.fftWorkBufFirst);
//		ippsConjPerm_32fc(ctx.fBuffer_Window, ctx.fcBuffer_FFT, nFFTLength);
//
//		// (3) Zero Pad & Reorder for IFFT
//		ippsZero_32fc(ctx.fcBuffer_IFFT, nFFTLength);
//		ippsCopy_32fc(ctx.fcBuffer_FFT, ctx.fcBuffer_IFFT, nOutputLength);
//
//		// (4) Inverse FFT
//		ippsFFTInv_CToC_32fc_I(ctx.fcBuffer_IFFT, ifftSpec, ctx.fftWorkBufIFFT);
//
//		// (5) 보간 전 fcBuffer_FFT 전체 초기화
//		ippsZero_32fc(ctx.fcBuffer_FFT, nFFTLength);
//
//		// (6) Interpolation with bounds check
//		int maxJ = std::min(nAScan / 2, nOutputLength - 1);
//		for (int j = 0; j < maxJ; ++j) {
//			int idx = calibration->indexMap[j];
//			if (idx + 1 < nFFTLength) {
//				float w = calibration->weightMap[j];
//				const Ipp32fc& s0 = ctx.fcBuffer_IFFT[idx];
//				const Ipp32fc& s1 = ctx.fcBuffer_IFFT[idx + 1];
//				ctx.fcBuffer_FFT[j].re = w * s0.re + (1.0f - w) * s1.re;
//				ctx.fcBuffer_FFT[j].im = w * s0.im + (1.0f - w) * s1.im;
//			}
//			else {
//				ctx.fcBuffer_FFT[j].re = 0;
//				ctx.fcBuffer_FFT[j].im = 0;
//			}
//		}
//
//		// (7) Numerical Dispersion Compensation
